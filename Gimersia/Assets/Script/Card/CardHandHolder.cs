...
public class CardHandHolder : MonoBehaviour
{
    ...
    private PlayerState currentOwner;
    ...
    public void SetOwner(PlayerState player)
    {
        currentOwner = player;
    }
    #endregion
    ...
    public bool SpawnCard(NewCardData data, int slotIndex)
    {
        if (slotIndex < 0 || slotIndex >= cardSlots.Count) return false;
        CardSlot slot = cardSlots[slotIndex];
        if (slot.HasCard()) return false;

        if (data == null || data.CardPrefab == null)
        {
            Debug.LogError($"Data kartu null atau Prefab belum di-assign di ScriptableObject {data?.name}");
            return false;
        }

        // Instantiate dari data.CardPrefab (Bukan prefab default holder)
        GameObject cardObj = Instantiate(data.CardPrefab, slot.transform);

        // Ambil komponen GameCard (Logic) & CardVisual (Animasi)
        BaseCard cardComponent = cardObj.GetComponent<BaseCard>(); // GameCard adalah turunan BaseCard
        CardVisual cardVisual = cardObj.GetComponent<CardVisual>();

        if (cardComponent == null || cardVisual == null)
        {
            Debug.LogError("Prefab kartu TIDAK punya script GameCard atau CardVisual!");
            Destroy(cardObj);
            return false;
        }

        // --- INJECT DATA & OWNER (PENTING!) ---
        cardComponent.Initialize(data, currentOwner);
        // --------------------------------------

        // Setup Visual
        cardVisual.Initialize(slot);

        // Update Teks & Icon Visual sesuai Data Logic
        cardVisual.SetCardData(data.CardName, data.CardDescription, data.CardIcon);

        slot.SetCard(cardVisual);

        return true;
    }
    public void SpawnCardInFirstEmptySlot(NewCardData cardData)
    {
